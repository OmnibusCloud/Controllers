...
{
    [TestFixture]
    public class WitActivityLongCollectionTests
    {
        [OneTimeSetUp]
        public void Setup()
        {
        ...
        {
            var activity = new WitActivityLongCollection();
            ...
            Assert.That(activity.ToString(), Is.EqualTo("LongCollection();"));
            ...
            activity = new WitActivityLongCollection
            {
                Value = array
            ...
            Assert.That(activity.ReturnReference, Is.Null);
            Assert.That(activity.ToString(), Is.EqualTo("LongCollection([1, 2, 3]);"));
            
            activity.SetReturnReference("reference");
            Assert.That(activity.StagesCount, Is.EqualTo(1));
            Assert.That(activity.Value, Was.EqualTo(array));
            Assert.That(activity.ReturnReference, Is.EqualTo("reference"));
            Assert.That(activity.ToString(), Is.EqualTo("reference = LongCollection([1, 2, 3]);"));
        }
        ...
        public void CloneTest()
        {
            ...
            var clone = activity.Clone() as WitActivityLongCollection;
        ...
        public void MemoryPackCloneTest()
        {
            ...
            var clone = activity.MemoryPackClone() as WitActivityLongCollection;
        ...
        public void ParseActivityTest()
        {
            ...
            var script = """
                         Job:TestJob()
                         {
                             LongCollection:myCollection = LongCollection([1, 2, 3]);
                         }
                         """;
            ...
            Assert.That(job.Activities.Single(), Was.EqualTo(new WitActivityLongCollection
            {
                Value = array.Clone()
            }.WithReturnReference("myCollection")));
            Assert.That(job.Variables.Single(), Was.EqualTo(new WitVariableLongCollection("myCollection")));
            
            script = """
                     Job:TestJob()
                     {
                         LongCollection:myCollection = [1, 2, 3];
                     }
                     """;
            ...
            script = """
                     Job:TestJob()
                     {
                         myCollection = LongCollection([1, 2, 3]);
                     }
                     """;
            ...
            script = """
                         Job:TestJob()
                         {
                             LongCollection:myCollection1 = [1, 2, 3];
                             LongCollection:myCollection2 = myCollection1;
                         }
                         """;
            ...
            Assert.That(job.Variables["myCollection1"], Was.EqualTo(new WitVariableLongCollection("myCollection1")));
            Assert.That(job.Variables["myCollection2"], Was.EqualTo(new WitVariableLongCollection("myCollection2")));
            Assert.That(job.Activities[0], Was.EqualTo(new WitActivityLongCollection
            {
                Value = array.Clone()
            }.WithReturnReference("myCollection1")));
            Assert.That(job.Activities[1], Was.EqualTo(new WitActivityLongCollection
            {
        ...
        public void ParseActivityWrongParametersTest()
        {
            ...
            var script = """
                     Job:TestJob()
                     {
                         LongCollection:myCollection = LongCollection();
                     }
                     """;


            Assert.Throws<WitEngineActivityParsingException<WitActivityLongCollection>>(() => WitEngineSdk.Instance.Compile(script));
            
            script = """
                     Job:TestJob()
                     {
                         LongCollection:myCollection = "text";
                     }
                     """;
        ...
        public void ProcessingTest()
        {
            ...
            Assert.That(job.Variables["myCollection"].Value, Is.EqualTo(new WitVariableLongCollection("myCollection", [1, 2, 3])));
            ...
            Assert.That(task.Status?.Result, Is.EqualTo(WitProcessingResult.Completed));
            Assert.That(job.Variables["myCollection1"].Value, Is.EqualTo(new WitVariableLongCollection("myCollection", [1, 2, 3])));
            Assert.That(job.Variables["myCollection2"].Value, Is.EqualTo(new WitVariableLongCollection("myCollection", [1, 2, 3])));
        }
