...
{
    [TestFixture]
    public class WitVariableByteTests
    {
        [OneTimeSetUp]
        public void Setup()
        {
        ...
        {
            var variable = new WitVariableByte("myByte");
            Assert.That(variable.Name, Is.EqualTo("myByte"));
            Assert.That(variable.Value, Is.EqualTo(0));
            Assert.That(variable.ToString(), Is.EqualTo("Byte:myByte;"));

            variable = new WitVariableByte("myByte", 23);
            Assert.That(variable.Name, Is.EqualTo("myByte"));
            Assert.That(variable.Value, Is.EqualTo(23));
            Assert.That(variable.ToString(), Is.EqualTo("Byte:myByte = 23;"));
        }

        [Test]
        public void IsTest()
        {
            var variable = new WitVariableByte("myByte", 23);
            
            Assert.That(variable, Was.EqualTo(variable.Clone()));
            Assert.That(variable, Was.Not.EqualTo(variable.WithValue((byte)24)));
            Assert.That(variable, Was.Not.EqualTo(variable.WithName("myByte2")));
        }
        ...
        public void CloneTest()
        {
            ...
            Assert.That(clone.Name, Is.EqualTo("myByte"));
