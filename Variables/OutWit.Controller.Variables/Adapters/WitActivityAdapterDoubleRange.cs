...
{
    internal sealed class WitActivityAdapterDoubleRange : WitActivityAdapterFunction<WitActivityDoubleRange>, IWitActivityAdapter<WitActivityDoubleRange>
    {
        #region Constructors

        public WitActivityAdapterDoubleRange(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityDoubleRange activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetValue(activity.From, out double from))
                throw this.FailedToGetParameterValueException(range => range.From);
            
            if (!pool.TryGetValue(activity.To, out double to))
                throw this.FailedToGetParameterValueException(range => range.To);
            
            if(to <= from)
                throw this.InvalidRangeException(activity.From, activity.To);

            if (!pool.TryGetValue(activity.Step, out double step))
                step = 1;
        ...
        protected override WitActivityDoubleRange CreateActivity(IWitParameter[] parameters)
        {
            try
            {
        ...
        private WitActivityDoubleRange CreateActivity(IWitParameter from, IWitParameter to)
        {
            ...
            return new WitActivityDoubleRange
            {
                From = from,
                To = to,
            };
        ...
        private WitActivityDoubleRange CreateActivity(IWitParameter from, IWitParameter to, IWitParameter step)
        {
            if (!from.IsNumericOrReference())
                throw this.ExpectedStringException(range => range.From);
