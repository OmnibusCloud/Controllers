...
{
    internal sealed class WitActivityAdapterUIntegerCollection : WitActivityAdapterFunction<WitActivityUIntegerCollection>, IWitActivityAdapter<WitActivityUIntegerCollection>
    {
        #region Constructors

        public WitActivityAdapterUIntegerCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityUIntegerCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<uint>? value) || value == null)
        ...
        protected override WitActivityUIntegerCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityUIntegerCollection { Value = parameters[0] };
