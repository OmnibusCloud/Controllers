...
{
    internal sealed class WitActivityAdapterIntegerRange : WitActivityAdapterFunction<WitActivityIntegerRange>, IWitActivityAdapter<WitActivityIntegerRange>
    {
        #region Constructors

        public WitActivityAdapterIntegerRange(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityIntegerRange activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetValue(activity.From, out int from))
                throw this.FailedToGetParameterValueException(range => range.From);
            
            if (!pool.TryGetValue(activity.To, out int to))
                throw this.FailedToGetParameterValueException(range => range.To);
            
            if(to <= from)
                throw this.InvalidRangeException(activity.From, activity.To);

            if (!pool.TryGetValue(activity.Step, out int step))
                step = 1;
        ...
        protected override WitActivityIntegerRange CreateActivity(IWitParameter[] parameters)
        {
            try
            {
        ...
        private WitActivityIntegerRange CreateActivity(IWitParameter from, IWitParameter to)
        {
            ...
            return new WitActivityIntegerRange
            {
                From = from,
                To = to,
            };
        ...
        private WitActivityIntegerRange CreateActivity(IWitParameter from, IWitParameter to, IWitParameter step)
        {
            if (!from.IsNumericOrReference())
                throw this.ExpectedStringException(range => range.From);
