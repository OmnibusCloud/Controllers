...
{
    internal sealed class WitActivityAdapterBooleanCollection : WitActivityAdapterFunction<WitActivityBooleanCollection>, IWitActivityAdapter<WitActivityBooleanCollection>
    {
        #region Constructors

        public WitActivityAdapterBooleanCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityBooleanCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<bool>? value) || value == null)
        ...
        protected override WitActivityBooleanCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityBooleanCollection { Value = parameters[0] };
