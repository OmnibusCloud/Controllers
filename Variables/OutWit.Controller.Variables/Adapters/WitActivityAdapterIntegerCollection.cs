...
{
    internal sealed class WitActivityAdapterIntegerCollection : WitActivityAdapterFunction<WitActivityIntegerCollection>, IWitActivityAdapter<WitActivityIntegerCollection>
    {
        #region Constructors

        public WitActivityAdapterIntegerCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityIntegerCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<int>? value) || value == null)
        ...
        protected override WitActivityIntegerCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityIntegerCollection { Value = parameters[0] };
