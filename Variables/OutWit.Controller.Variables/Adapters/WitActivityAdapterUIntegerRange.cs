...
{
    internal sealed class WitActivityAdapterUIntegerRange : WitActivityAdapterFunction<WitActivityUIntegerRange>, IWitActivityAdapter<WitActivityUIntegerRange>
    {
        #region Constructors

        public WitActivityAdapterUIntegerRange(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityUIntegerRange activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetValue(activity.From, out uint from))
                throw this.FailedToGetParameterValueException(range => range.From);
            
            if (!pool.TryGetValue(activity.To, out uint to))
                throw this.FailedToGetParameterValueException(range => range.To);
            
            if(to <= from)
                throw this.InvalidRangeException(activity.From, activity.To);

            if (!pool.TryGetValue(activity.Step, out uint step))
                step = 1;
            ...
            var values = new List<uint>(capacity);
        ...
        protected override WitActivityUIntegerRange CreateActivity(IWitParameter[] parameters)
        {
            try
            {
        ...
        private WitActivityUIntegerRange CreateActivity(IWitParameter from, IWitParameter to)
        {
            ...
            return new WitActivityUIntegerRange
            {
                From = from,
                To = to,
            };
        ...
        private WitActivityUIntegerRange CreateActivity(IWitParameter from, IWitParameter to, IWitParameter step)
        {
            if (!from.IsNumericOrReference())
                throw this.ExpectedStringException(range => range.From);
