...
{
    internal sealed class WitActivityAdapterStringCollection : WitActivityAdapterFunction<WitActivityStringCollection>, IWitActivityAdapter<WitActivityStringCollection>
    {
        #region Constructors

        public WitActivityAdapterStringCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityStringCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<string?>? value) || value == null)
        ...
        protected override WitActivityStringCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityStringCollection { Value = parameters[0] };
