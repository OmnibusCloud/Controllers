...
{
    internal sealed class WitActivityAdapterShortRange : WitActivityAdapterFunction<WitActivityShortRange>, IWitActivityAdapter<WitActivityShortRange>
    {
        #region Constructors

        public WitActivityAdapterShortRange(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityShortRange activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetValue(activity.From, out short from))
                throw this.FailedToGetParameterValueException(range => range.From);
            
            if (!pool.TryGetValue(activity.To, out short to))
                throw this.FailedToGetParameterValueException(range => range.To);
            
            if(to <= from)
                throw this.InvalidRangeException(activity.From, activity.To);

            if (!pool.TryGetValue(activity.Step, out short step))
                step = 1;
            ...
            var values = new List<short>(capacity);
            for(short i = from; i < to; i += step)
                values.Add(i);
        ...
        protected override WitActivityShortRange CreateActivity(IWitParameter[] parameters)
        {
            try
            {
        ...
        private WitActivityShortRange CreateActivity(IWitParameter from, IWitParameter to)
        {
            ...
            return new WitActivityShortRange
            {
                From = from,
                To = to,
            };
        ...
        private WitActivityShortRange CreateActivity(IWitParameter from, IWitParameter to, IWitParameter step)
        {
            if (!from.IsNumericOrReference())
                throw this.ExpectedStringException(range => range.From);
