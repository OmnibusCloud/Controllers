...
{
    internal sealed class WitActivityAdapterShortCollection : WitActivityAdapterFunction<WitActivityShortCollection>, IWitActivityAdapter<WitActivityShortCollection>
    {
        #region Constructors

        public WitActivityAdapterShortCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityShortCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<short>? value) || value == null)
        ...
        protected override WitActivityShortCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityShortCollection { Value = parameters[0] };
