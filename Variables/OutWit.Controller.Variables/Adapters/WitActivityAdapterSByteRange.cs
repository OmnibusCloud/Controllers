...
{
    internal sealed class WitActivityAdapterSByteRange : WitActivityAdapterFunction<WitActivitySByteRange>, IWitActivityAdapter<WitActivitySByteRange>
    {
        #region Constructors

        public WitActivityAdapterSByteRange(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivitySByteRange activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetValue(activity.From, out sbyte from))
                throw this.FailedToGetParameterValueException(range => range.From);
            
            if (!pool.TryGetValue(activity.To, out sbyte to))
                throw this.FailedToGetParameterValueException(range => range.To);
            
            if(to <= from)
                throw this.InvalidRangeException(activity.From, activity.To);

            if (!pool.TryGetValue(activity.Step, out sbyte step))
                step = 1;
            ...
            var values = new List<sbyte>(capacity);
            for(sbyte i = from; i < to; i += step)
                values.Add(i);
        ...
        protected override WitActivitySByteRange CreateActivity(IWitParameter[] parameters)
        {
            try
            {
        ...
        private WitActivitySByteRange CreateActivity(IWitParameter from, IWitParameter to)
        {
            ...
            return new WitActivitySByteRange
            {
                From = from,
                To = to,
            };
        ...
        private WitActivitySByteRange CreateActivity(IWitParameter from, IWitParameter to, IWitParameter step)
        {
            if (!from.IsNumericOrReference())
                throw this.ExpectedStringException(range => range.From);
