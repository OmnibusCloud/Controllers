...
{
    internal sealed class WitActivityAdapterFloatCollection : WitActivityAdapterFunction<WitActivityFloatCollection>, IWitActivityAdapter<WitActivityFloatCollection>
    {
        #region Constructors

        public WitActivityAdapterFloatCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityFloatCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<float>? value) || value == null)
        ...
        protected override WitActivityFloatCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityFloatCollection { Value = parameters[0] };
