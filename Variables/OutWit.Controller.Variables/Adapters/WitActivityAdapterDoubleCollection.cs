...
{
    internal sealed class WitActivityAdapterDoubleCollection : WitActivityAdapterFunction<WitActivityDoubleCollection>, IWitActivityAdapter<WitActivityDoubleCollection>
    {
        #region Constructors

        public WitActivityAdapterDoubleCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityDoubleCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<double>? value) || value == null)
        ...
        protected override WitActivityDoubleCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityDoubleCollection { Value = parameters[0] };
