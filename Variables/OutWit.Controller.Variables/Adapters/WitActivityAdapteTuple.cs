...
{
    internal sealed class WitActivityAdapterTuple : WitActivityAdapterFunction<WitActivityTuple>, IWitActivityAdapter<WitActivityTuple>
    {
        #region Constructors

        public WitActivityAdapterTuple(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityTuple activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
        ...
        protected override WitActivityTuple CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityTuple { Value = parameters[0] };
