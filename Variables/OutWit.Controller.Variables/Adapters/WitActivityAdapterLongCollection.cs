...
{
    internal sealed class WitActivityAdapterLongCollection : WitActivityAdapterFunction<WitActivityLongCollection>, IWitActivityAdapter<WitActivityLongCollection>
    {
        #region Constructors

        public WitActivityAdapterLongCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityLongCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<long>? value) || value == null)
        ...
        protected override WitActivityLongCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityLongCollection { Value = parameters[0] };
