...
{
    internal sealed class WitActivityAdapterUShortCollection : WitActivityAdapterFunction<WitActivityUShortCollection>, IWitActivityAdapter<WitActivityUShortCollection>
    {
        #region Constructors

        public WitActivityAdapterUShortCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityUShortCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<ushort>? value) || value == null)
        ...
        protected override WitActivityUShortCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityUShortCollection { Value = parameters[0] };
