...
{
    internal sealed class WitActivityAdapterSByteCollection : WitActivityAdapterFunction<WitActivitySByteCollection>, IWitActivityAdapter<WitActivitySByteCollection>
    {
        #region Constructors

        public WitActivityAdapterSByteCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivitySByteCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<sbyte>? value) || value == null)
        ...
        protected override WitActivitySByteCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivitySByteCollection { Value = parameters[0] };
