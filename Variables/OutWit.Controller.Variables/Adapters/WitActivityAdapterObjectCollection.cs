...
{
    internal sealed class WitActivityAdapterObjectCollection : WitActivityAdapterFunction<WitActivityObjectCollection>, IWitActivityAdapter<WitActivityObjectCollection>
    {
        #region Constructors

        public WitActivityAdapterObjectCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityObjectCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
        ...
        protected override WitActivityObjectCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityObjectCollection { Value = parameters[0] };
