...
{
    internal sealed class WitActivityAdapterDecimalCollection : WitActivityAdapterFunction<WitActivityDecimalCollection>, IWitActivityAdapter<WitActivityDecimalCollection>
    {
        #region Constructors

        public WitActivityAdapterDecimalCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityDecimalCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<decimal>? value) || value == null)
        ...
        protected override WitActivityDecimalCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityDecimalCollection { Value = parameters[0] };
