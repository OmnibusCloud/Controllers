...
{
    internal sealed class WitActivityAdapterDateTimeCollection : WitActivityAdapterFunction<WitActivityDateTimeCollection>, IWitActivityAdapter<WitActivityDateTimeCollection>
    {
        #region Constructors

        public WitActivityAdapterDateTimeCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityDateTimeCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            IReadOnlyList<DateTime?>? collection;
        ...
        private IReadOnlyList<DateTime?> BuildCollection(IWitVariablesCollection pool, IReadOnlyList<object?> value)
        {
            ...
            foreach (var color in value)
            {
                switch (color)
                {
                    case string constant:
                        if (DateTime.TryParse(constant, CultureInfo.InvariantCulture, DateTimeStyles.None, out var parsedDateTime))
                    ...
                    case IWitReference reference:
                        if (pool.TryGetValue(reference, out DateTime? dateTimeObject))
        ...
        protected override WitActivityDateTimeCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityDateTimeCollection { Value = parameters[0] };
