...
{
    internal sealed class WitActivityAdapterULongRange : WitActivityAdapterFunction<WitActivityULongRange>, IWitActivityAdapter<WitActivityULongRange>
    {
        #region Constructors

        public WitActivityAdapterULongRange(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityULongRange activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetValue(activity.From, out ulong from))
                throw this.FailedToGetParameterValueException(range => range.From);
            
            if (!pool.TryGetValue(activity.To, out ulong to))
                throw this.FailedToGetParameterValueException(range => range.To);
            
            if(to <= from)
                throw this.InvalidRangeException(activity.From, activity.To);

            if (!pool.TryGetValue(activity.Step, out ulong step))
                step = 1;
            ...
            var values = new List<ulong>(capacity);
            for(ulong i = from; i < to; i += step)
                values.Add(i);
        ...
        protected override WitActivityULongRange CreateActivity(IWitParameter[] parameters)
        {
            try
            {
        ...
        private WitActivityULongRange CreateActivity(IWitParameter from, IWitParameter to)
        {
            ...
            return new WitActivityULongRange
            {
                From = from,
                To = to,
            };
        ...
        private WitActivityULongRange CreateActivity(IWitParameter from, IWitParameter to, IWitParameter step)
        {
            if (!from.IsNumericOrReference())
                throw this.ExpectedStringException(range => range.From);
