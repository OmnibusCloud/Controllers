...
{
    internal sealed class WitActivityAdapterByteCollection : WitActivityAdapterFunction<WitActivityByteCollection>, IWitActivityAdapter<WitActivityByteCollection>
    {
        #region Constructors

        public WitActivityAdapterByteCollection(IWitProcessingManager processingManager, IResources resources, ILogger logger) :
        ...
        protected override async Task Process(WitActivityByteCollection activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
        {
            if (!pool.TryGetCollection(activity.Value, out IReadOnlyList<byte>? value) || value == null)
        ...
        protected override WitActivityByteCollection CreateActivity(IWitParameter[] parameters)
        {
            try
            {
                ...
                if (parameters[0].IsArrayOrReference())
                    return new WitActivityByteCollection { Value = parameters[0] };
