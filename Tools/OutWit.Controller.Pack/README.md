...
`.NET` global tool for third-party controller authors. Packages a built controller `.module/` directory + its `controller.json` manifest into a self-contained zip ready for upload to an OmnibusCloud server via the admin UI.
