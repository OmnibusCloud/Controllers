...
internal sealed class WitActivityAdapterSchwarzMakeFinalTasks : WitActivityAdapterFunction<WitActivitySchwarzMakeFinalTasks>
{
    #region Constructors

    public WitActivityAdapterSchwarzMakeFinalTasks(IWitProcessingManager processingManager, ILogger logger)
    ...
    protected override async Task Process(WitActivitySchwarzMakeFinalTasks activity, IWitVariablesCollection pool, IWitActivityStatus? activityStatus, WitProcessingStatus status)
    {
        ...
        var tasks = SchwarzTaskFactory.BuildTasks(plan, state, emitField: true);
    ...
    protected override WitActivitySchwarzMakeFinalTasks CreateActivity(IWitParameter[] parameters)
    {
        try
        {
