...
/// <summary>
/// Three in-process "nodes" over the single WitEngineNodeSdk instance —
/// the Matrices/Grid mock pattern: real task building and serialization
/// paths, no real network distribution.
/// </summary>
internal sealed class SimulationTestNodesManager : IWitNodesManager
{
    #region Fields
    ...
    public SimulationTestNodesManager(IWitEngineNode node)
    {
        m_node = node;
        CompatibleNodes =
        [
            new SimulationTestActivityNode(node),
            new SimulationTestActivityNode(node),
            new SimulationTestActivityNode(node)
        ];
    }
