...
/// <summary>
/// Collection of variant run results - the wave a chunk's Grid.ForEach
/// returns, in completion order (never source order: map by VariantIndex).
/// </summary>
[Variable("FoamResultCollection")]
[MemoryPackable]
public sealed partial class WitVariableFoamResultCollection : WitCollection<FoamResultData?>, IWitVariableFactory<WitVariableFoamResultCollection>
{
    #region Constructors
    ...
    public WitVariableFoamResultCollection(string name)
        : base(name)
    {
    }
    ...
    public WitVariableFoamResultCollection(string name, IReadOnlyList<FoamResultData?> value)
        : base(name, value)
    {
    }
    ...
    {
        if (modelBase is not WitVariableFoamResultCollection variable)
            return false;
    ...
    public override WitVariableFoamResultCollection Clone()
    {
        var clonedItems = GetValue()?
            .Select(x => x?.Clone())
            .ToArray() ?? [];

        return new WitVariableFoamResultCollection(Name, clonedItems);
    }
    ...
    public static WitVariableFoamResultCollection Create(string name)
    {
        return new WitVariableFoamResultCollection(name);
    }
