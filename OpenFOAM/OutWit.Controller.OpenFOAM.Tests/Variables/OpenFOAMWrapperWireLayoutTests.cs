...
namespace OutWit.Controller.OpenFOAM.Tests.Variables;
...
/// (those are frozen by <c>OpenFOAMModelWireLayoutTests</c>). The wrappers
...
/// </summary>
[TestFixture]
public sealed class OpenFOAMWrapperWireLayoutTests
{
    // Declared layouts frozen 2026-09-24 (controller 0.1.0). The variable
    // wrappers deliberately declare NOTHING - name and payload serialize
    // through the engine base class; a property declared here would silently
    // extend the wire.
    private static readonly IReadOnlyDictionary<Type, string[]> EXPECTED_LAYOUTS = new Dictionary<Type, string[]>
    {
        [typeof(WitVariableFoamTask)] = [],
        [typeof(WitVariableFoamTaskCollection)] = [],
        [typeof(WitVariableFoamResult)] = [],
        [typeof(WitVariableFoamResultCollection)] = [],
        [typeof(WitActivityFoamRun)] = ["Task"]
    };
    ...
    public void EveryWrapperTypeIsFrozenTest()
    {
        ...
        foreach (var type in typeof(WitActivityFoamRun).Assembly.GetTypes())
        {
