...
/// <summary>
/// Three in-process "nodes" over the single WitEngineNodeSdk instance - the
/// Matrices/Grid/Sweep mock pattern: real task building and serialization
/// paths, no real network distribution.
/// </summary>
internal sealed class FoamTestNodesManager : IWitNodesManager
{
    #region Fields
    ...
    public FoamTestNodesManager(IWitEngineNode node)
    {
        m_node = node;
        CompatibleNodes =
        [
            new FoamTestActivityNode(),
            new FoamTestActivityNode(),
            new FoamTestActivityNode()
        ];
    }
