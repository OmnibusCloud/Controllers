...
/// <summary>
/// Collection of variant solve tasks — output of the Sweep chunk maker, input
/// to Grid.ForEach.
/// </summary>
[Variable("CcxTaskCollection")]
[MemoryPackable]
public sealed partial class WitVariableCcxTaskCollection : WitCollection<CcxTaskData?>, IWitVariableFactory<WitVariableCcxTaskCollection>
{
    #region Constructors
    ...
    public WitVariableCcxTaskCollection(string name)
        : base(name)
    {
    }
    ...
    public WitVariableCcxTaskCollection(string name, IReadOnlyList<CcxTaskData?> value)
        : base(name, value)
    {
    }
    ...
    {
        if (modelBase is not WitVariableCcxTaskCollection variable)
            return false;
    ...
    public override WitVariableCcxTaskCollection Clone()
    {
        var clonedItems = GetValue()?
            .Select(x => x?.Clone())
            .ToArray() ?? [];

        return new WitVariableCcxTaskCollection(Name, clonedItems);
    }
    ...
    public static WitVariableCcxTaskCollection Create(string name)
    {
        return new WitVariableCcxTaskCollection(name);
    }
