...
If the dotnet-debug MCP (`user-dotnet-debug`) is connected to the harness, you
can use it to debug .NET targets: set breakpoints, launch under `netcoredbg`,
inspect the call stack and variables, step into/over/out, and continue/stop.

### When to use

- You need to verify execution up to a breakpoint, look at the call stack or
  variables, or step through code.
- The target is a `.dll` or `.exe` (a built project). For stable debugging,
  build the target in Debug, not Release; pass the path to the Debug build
  (e.g. `bin/Debug/net10.0/App.dll`) to `debug_launch` and
  `debug_set_breakpoints`.

### Typical flow

1. `debug_set_breakpoints` — `workspace_path`, `target_path` (path to the
   `.dll` / `.exe`), `breakpoints[]` (`file_path`, `line`; optionally
   `condition`).
2. `debug_launch` — same `workspace_path` and `target_path`; pass
   `program_args` (an array of strings) when the program would otherwise exit
   immediately (e.g. `["dummy"]` for a console app that requires CLI input).
3. After launch the server waits for the first `stopped` event (up to 5 s).
   From there: `debug_stack_trace`, `debug_variables` (variables via DAP
   scopes), `debug_step_over` / `debug_step_into` / `debug_step_out`,
   `debug_continue`, and finally `debug_stop`.

Variables are fetched via scopes (`scopes` by `frameId` → `variables` by
`variablesReference`); on error a fallback to `variables(frameId)` is used.
