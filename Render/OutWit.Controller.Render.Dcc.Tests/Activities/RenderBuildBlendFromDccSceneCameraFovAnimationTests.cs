...
[TestFixture]
public sealed class RenderBuildBlendFromDccSceneCameraFovAnimationTests
{
    #region Fields
    ...
    public void SetUp()
    {
        m_storageDir = Path.Combine(Path.GetTempPath(), $"witcloud_render_buildblend_camera_fov_animation_dcc_test_{Guid.NewGuid():N}");
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithCameraFovAnimationThenRenderFramesCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc camera-FOV frame-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderCameraFovAnimatedFrames(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options)
                     {
        ...
        var keyframe1 = DccRenderTestData.CreateCameraFovKeyframe(1, 45d);
        keyframe1.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe2 = DccRenderTestData.CreateCameraFovKeyframe(2, 60d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        camera.VerticalFovKeyframes = [keyframe1, keyframe2];
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithCameraFovAnimationThenRenderVideoCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc camera-FOV video-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderCameraFovAnimatedVideo(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options, VideoOptions:video)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
        ...
        var keyframe2 = DccRenderTestData.CreateCameraFovKeyframe(2, 60d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe3 = DccRenderTestData.CreateCameraFovKeyframe(3, 35d);
        keyframe3.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        camera.VerticalFovKeyframes = [keyframe1, keyframe2, keyframe3];
        scene.Cameras.Add(camera);
        scene.Nodes.Add(DccRenderTestData.CreateCameraNode());
