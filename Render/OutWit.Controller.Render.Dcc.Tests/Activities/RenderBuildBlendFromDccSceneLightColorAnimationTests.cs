...
[TestFixture]
public sealed class RenderBuildBlendFromDccSceneLightColorAnimationTests
{
    #region Fields
    ...
    public void SetUp()
    {
        m_storageDir = Path.Combine(Path.GetTempPath(), $"witcloud_render_buildblend_light_color_animation_dcc_test_{Guid.NewGuid():N}");
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithLightColorAnimationThenRenderFramesCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc light-color frame-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderLightColorAnimatedFrames(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options)
                     {
        ...
        var keyframe1 = DccRenderTestData.CreateLightColorKeyframe(1, 1d, 0.95d, 0.85d);
        keyframe1.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe2 = DccRenderTestData.CreateLightColorKeyframe(2, 0.2d, 0.3d, 1d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        light.ColorKeyframes = [keyframe1, keyframe2];
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithLightColorAnimationThenRenderVideoCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc light-color video-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderLightColorAnimatedVideo(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options, VideoOptions:video)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
        ...
        var keyframe2 = DccRenderTestData.CreateLightColorKeyframe(2, 0.2d, 0.3d, 1d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe3 = DccRenderTestData.CreateLightColorKeyframe(3, 1d, 0.8d, 0.2d);
        keyframe3.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        light.ColorKeyframes = [keyframe1, keyframe2, keyframe3];
        scene.Lights.Add(light);
        scene.Nodes.Add(DccRenderTestData.CreateLightNode());
