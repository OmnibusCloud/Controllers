...
[TestFixture]
public sealed class RenderBuildBlendFromDccSceneCombinedAnimationTests
{
    #region Fields
    ...
    public void SetUp()
    {
        m_storageDir = Path.Combine(Path.GetTempPath(), $"witcloud_render_buildblend_combined_animation_dcc_test_{Guid.NewGuid():N}");
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithTransformAndVisibilityAnimationThenRenderFramesCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc combined-animation frame-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderCombinedAnimatedFrames(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options)
                     {
        ...
        var transformKeyframe1 = DccRenderTestData.CreateTransformKeyframe(1, 1d, 2d, 3d);
        transformKeyframe1.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var transformKeyframe2 = DccRenderTestData.CreateTransformKeyframe(2, 3d, 4d, 5d);
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithTransformAndVisibilityAnimationThenRenderVideoCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc combined-animation video-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderCombinedAnimatedVideo(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options, VideoOptions:video)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
        ...
        var transformKeyframe2 = DccRenderTestData.CreateTransformKeyframe(2, 3d, 4d, 5d);
        transformKeyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var transformKeyframe3 = DccRenderTestData.CreateTransformKeyframe(3, 5d, 4d, 3d);
