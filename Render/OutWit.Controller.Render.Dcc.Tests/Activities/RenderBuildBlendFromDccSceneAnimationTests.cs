...
[TestFixture]
public sealed class RenderBuildBlendFromDccSceneAnimationTests
{
    #region Fields
    ...
    public void SetUp()
    {
        m_storageDir = Path.Combine(Path.GetTempPath(), $"witcloud_render_buildblend_animation_dcc_test_{Guid.NewGuid():N}");
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithTransformAnimationThenRenderFramesCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc animation frame-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderAnimatedFrames(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options)
                     {
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithCameraTransformAnimationThenRenderVideoCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc camera-animation video-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderAnimatedCameraVideo(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options, VideoOptions:video)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
        ...
        [
            DccRenderTestData.CreateTransformKeyframe(1, 5d, -5d, 3d),
            DccRenderTestData.CreateTransformKeyframe(2, 6d, -4d, 3d),
            DccRenderTestData.CreateTransformKeyframe(3, 7d, -3d, 3d)
        ];
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithLightTransformAnimationThenRenderVideoCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc light-animation video-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderAnimatedLightVideo(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options, VideoOptions:video)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
        ...
        scene.Nodes[^1].TransformKeyframes =
        [
            DccRenderTestData.CreateTransformKeyframe(1, 4d, -4d, 6d),
            DccRenderTestData.CreateTransformKeyframe(2, 5d, -3d, 7d),
            DccRenderTestData.CreateTransformKeyframe(3, 6d, -2d, 8d)
        ];
        scene.RenderSettings.FrameEnd = 3;
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithCameraTransformAnimationThenRenderFramesCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc camera-animation frame-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderAnimatedCameraFrames(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithLightTransformAnimationThenRenderFramesCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc light-animation frame-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderAnimatedLightFrames(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithTransformAnimationThenRenderVideoCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc animation video-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderAnimatedVideo(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options, VideoOptions:video)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
        ...
        [
            DccRenderTestData.CreateTransformKeyframe(1, 1d, 2d, 3d),
            DccRenderTestData.CreateTransformKeyframe(2, 3d, 4d, 5d),
            DccRenderTestData.CreateTransformKeyframe(3, 5d, 4d, 3d)
        ];
        scene.RenderSettings.FrameEnd = 3;
