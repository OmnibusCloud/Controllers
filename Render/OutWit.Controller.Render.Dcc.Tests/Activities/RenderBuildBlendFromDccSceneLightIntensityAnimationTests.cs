...
[TestFixture]
public sealed class RenderBuildBlendFromDccSceneLightIntensityAnimationTests
{
    #region Fields
    ...
    public void SetUp()
    {
        m_storageDir = Path.Combine(Path.GetTempPath(), $"witcloud_render_buildblend_light_intensity_animation_dcc_test_{Guid.NewGuid():N}");
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithLightIntensityAnimationThenRenderFramesCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc light-intensity frame-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderLightIntensityAnimatedFrames(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options)
                     {
        ...
        var keyframe1 = DccRenderTestData.CreateLightIntensityKeyframe(1, 1200d);
        keyframe1.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe2 = DccRenderTestData.CreateLightIntensityKeyframe(2, 400d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        light.IntensityKeyframes = [keyframe1, keyframe2];
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithLightIntensityAnimationThenRenderVideoCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc light-intensity video-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderLightIntensityAnimatedVideo(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options, VideoOptions:video)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
        ...
        var keyframe2 = DccRenderTestData.CreateLightIntensityKeyframe(2, 400d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe3 = DccRenderTestData.CreateLightIntensityKeyframe(3, 900d);
        keyframe3.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        light.IntensityKeyframes = [keyframe1, keyframe2, keyframe3];
        scene.Lights.Add(light);
        scene.Nodes.Add(DccRenderTestData.CreateLightNode());
