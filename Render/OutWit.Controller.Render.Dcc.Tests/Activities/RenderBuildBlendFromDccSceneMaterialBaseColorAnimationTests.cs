...
[TestFixture]
public sealed class RenderBuildBlendFromDccSceneMaterialBaseColorAnimationTests
{
    #region Fields
    ...
    public void SetUp()
    {
        m_storageDir = Path.Combine(Path.GetTempPath(), $"witcloud_render_buildblend_material_base_color_animation_dcc_test_{Guid.NewGuid():N}");
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithMaterialBaseColorAnimationThenRenderFramesCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc material-base-color frame-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderMaterialBaseColorAnimatedFrames(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options)
                     {
        ...
        var keyframe1 = DccRenderTestData.CreateMaterialBaseColorKeyframe(1, 0.8d, 0.7d, 0.6d);
        keyframe1.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe2 = DccRenderTestData.CreateMaterialBaseColorKeyframe(2, 0.2d, 0.3d, 1d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        scene.Materials[0].BaseColorKeyframes = [keyframe1, keyframe2];
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithMaterialBaseColorAnimationThenRenderVideoCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc material-base-color video-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderMaterialBaseColorAnimatedVideo(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options, VideoOptions:video)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
        ...
        var keyframe2 = DccRenderTestData.CreateMaterialBaseColorKeyframe(2, 0.2d, 0.3d, 1d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe3 = DccRenderTestData.CreateMaterialBaseColorKeyframe(3, 1d, 0.8d, 0.2d);
        keyframe3.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        scene.Materials[0].BaseColorKeyframes = [keyframe1, keyframe2, keyframe3];
