...
[TestFixture]
public sealed class RenderBuildBlendFromDccSceneOpacityTextureTests
{
    #region Constants
    ...
    public void SetUp()
    {
        m_storageDir = Path.Combine(Path.GetTempPath(), $"witcloud_render_buildblend_opacity_dcc_test_{Guid.NewGuid():N}");
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithOpacityTextureAttachmentThenRenderStillCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc opacity-textured still-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderOpacityTexturedStill(DccScene:scene, Int:frame, RenderOptions:options)
                     {
