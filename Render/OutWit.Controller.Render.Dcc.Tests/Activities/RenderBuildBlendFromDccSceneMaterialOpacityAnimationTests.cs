...
[TestFixture]
public sealed class RenderBuildBlendFromDccSceneMaterialOpacityAnimationTests
{
    #region Fields
    ...
    public void SetUp()
    {
        m_storageDir = Path.Combine(Path.GetTempPath(), $"witcloud_render_buildblend_material_opacity_animation_dcc_test_{Guid.NewGuid():N}");
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithMaterialOpacityAnimationThenRenderFramesCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc material-opacity frame-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderMaterialOpacityAnimatedFrames(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options)
                     {
        ...
        var keyframe1 = DccRenderTestData.CreateMaterialOpacityKeyframe(1, 1d);
        keyframe1.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe2 = DccRenderTestData.CreateMaterialOpacityKeyframe(2, 0.5d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        scene.Materials[0].OpacityKeyframes = [keyframe1, keyframe2];
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithMaterialOpacityAnimationThenRenderVideoCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc material-opacity video-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderMaterialOpacityAnimatedVideo(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options, VideoOptions:video)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
        ...
        var keyframe2 = DccRenderTestData.CreateMaterialOpacityKeyframe(2, 0.5d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe3 = DccRenderTestData.CreateMaterialOpacityKeyframe(3, 0.8d);
        keyframe3.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        scene.Materials[0].OpacityKeyframes = [keyframe1, keyframe2, keyframe3];
