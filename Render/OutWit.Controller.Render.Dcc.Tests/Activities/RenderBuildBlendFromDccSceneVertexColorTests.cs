...
[TestFixture]
public sealed class RenderBuildBlendFromDccSceneVertexColorTests
{
    #region Fields
    ...
    public void SetUp()
    {
        m_storageDir = Path.Combine(Path.GetTempPath(), $"witcloud_render_buildblend_vertexcolor_dcc_test_{Guid.NewGuid():N}");
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithVertexColorsThenRenderStillCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc vertex-colour still-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderVertexColoredStill(DccScene:scene, Int:frame, RenderOptions:options)
                     {
        ...
        // No textures/images so only geometry + the vertex-colour attribute matter.
        scene.Materials[0].TextureSlots.Clear();
        scene.ImageAssets.Clear();
        scene.AttachedFiles.Clear();

        // One colour per mesh corner (aligned with Positions, as the generator indexes by vertex).
        scene.Meshes[0].Colors =
        [
            .. scene.Meshes[0].Positions.Select((_, index) => new DccColorData
            {
                R = index % 3 == 0 ? 1d : 0.2d,
                G = index % 3 == 1 ? 1d : 0.2d,
                B = index % 3 == 2 ? 1d : 0.2d,
        ...
        // The .blend with a BYTE_COLOR vertex-colour attribute must build and render on Blender 5.1.
