...
[TestFixture]
public sealed class RenderBuildBlendFromDccSceneEnvironmentWorldTests
{
    #region Constants
    ...
    public void SetUp()
    {
        m_storageDir = Path.Combine(Path.GetTempPath(), $"witcloud_render_buildblend_envworld_dcc_test_{Guid.NewGuid():N}");
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithEnvironmentImageThenRenderStillCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc environment-world still-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderEnvironmentStill(DccScene:scene, Int:frame, RenderOptions:options)
                     {
        ...
        // Drop the material textures/images so only the environment image needs materializing.
        scene.Materials[0].TextureSlots.Clear();
        scene.ImageAssets.Clear();
        scene.AttachedFiles.Clear();

        var envPath = Path.Combine(m_storageDir, "env.png");
        File.WriteAllBytes(envPath, Convert.FromBase64String(MINIMAL_PNG_BASE64));
        var envBlobId = m_blobService.RegisterExistingFile(envPath);

        scene.ImageAssets.Add(new DccImageAssetData
        {
            Id = "image:env",
            Name = "Env",
            SourcePath = "C:/textures/env.png",
            RelativePath = "textures/env.png",
            AssetKind = "ImageAsset"
        });
        scene.AttachedFiles.Add(DccRenderTestData.CreateImageAttachment(envBlobId, "C:/textures/env.png", "textures/env.png"));
        scene.World = new DccWorldData
        {
            EnvironmentImageId = "image:env",
            Strength = 5d,
            EnvironmentRotationDegrees = 30d
        };

        var status = await hostEngine.ScheduleAndWaitAsync(job, scene, 1, CreateRenderOptions());

        // The .blend with an Environment Texture world node must build and render on Blender 5.1.
