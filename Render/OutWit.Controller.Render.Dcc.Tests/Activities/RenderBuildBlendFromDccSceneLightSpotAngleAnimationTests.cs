...
[TestFixture]
public sealed class RenderBuildBlendFromDccSceneLightSpotAngleAnimationTests
{
    #region Fields
    ...
    public void SetUp()
    {
        m_storageDir = Path.Combine(Path.GetTempPath(), $"witcloud_render_buildblend_light_spot_angle_animation_dcc_test_{Guid.NewGuid():N}");
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithSpotAngleAnimationThenRenderFramesCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc spot-angle frame-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderSpotAngleAnimatedFrames(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options)
                     {
        ...
        var keyframe1 = DccRenderTestData.CreateSpotLightAngleKeyframe(1, 35d);
        keyframe1.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe2 = DccRenderTestData.CreateSpotLightAngleKeyframe(2, 20d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        light.SpotAngleKeyframes = [keyframe1, keyframe2];
    ...
    [Test]
    public async Task BuildBlendFromDccSceneWithSpotAngleAnimationThenRenderVideoCompletesTest()
    {
        if (RenderTestAssetPaths.FindRenderBlenderRoot() == null)
            Assert.Ignore("Packaged Blender runtime not found for RenderDcc spot-angle video-render integration test.");
        ...
        var script = """
                     Job:BuildAndRenderSpotAngleAnimatedVideo(DccScene:scene, Int:startFrame, Int:endFrame, RenderOptions:options, VideoOptions:video)
                     {
                         Blob:blend = Render.BuildBlendFromDccScene(scene);
                         RenderTaskCollection:tasks = Render.Split(blend, startFrame, endFrame, options);
                         RenderResultCollection:rendered = Grid.ForEach(task in tasks)
        ...
        var keyframe2 = DccRenderTestData.CreateSpotLightAngleKeyframe(2, 20d);
        keyframe2.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        var keyframe3 = DccRenderTestData.CreateSpotLightAngleKeyframe(3, 45d);
        keyframe3.InterpolationMode = DccKeyframeInterpolationMode.Linear;
        light.SpotAngleKeyframes = [keyframe1, keyframe2, keyframe3];
        scene.Lights.Add(light);
        scene.Nodes.Add(DccRenderTestData.CreateSpotLightNode());
