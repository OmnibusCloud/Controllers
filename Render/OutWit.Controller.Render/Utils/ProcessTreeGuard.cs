...
/// <summary>
/// Ties spawned render child processes (Blender / ffmpeg) to the lifetime of THIS process, so a
/// crashed or force-killed worker cannot leave orphans pinning the Provider's GPU/CPU.
...
/// the cancellation path still kills the tree explicitly; the POSIX orphan gap is documented in
/// Roadmap-v2 §17.
/// </summary>
internal static class ProcessTreeGuard
{
    ...
    /// effort: a failure is logged but never breaks the render (the explicit cancellation kill
    /// path is unaffected).
    /// </summary>
    ...
    private static nint EnsureJob(ILogger? logger)
    {
        lock (m_lock)
        {
            ...
            if (m_jobHandle == 0)
            {
                ...
                logger?.LogWarning(
                    "Kill-on-close job object could not be created (error {Error}); spawned render processes may orphan if this process dies",
