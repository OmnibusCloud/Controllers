...
**Revision 4:** 2026-06-30 — delegated bake (`Render.BakeSimulation` via `Grid.Delegate`) shipped + proven live for fluid (gas+liquid), point-cache sims (cloth/particles/soft body/dynamic paint/rigid body), and Geometry-Nodes simulation zones; full `BakeAndRender{Still,Frames,Video}{Cycles,Eevee,GreasePencil}` + `StillTiled×3` script family (Render 1.23.8 / Model 1.6.1 / Render.Scripts 1.4.0). Controller audited + hardened. Next: add-on/bridge integration — see §0.2.
**Revision 5:** 2026-07-11 — **the DCC (3ds Max) path is functionally production-complete** (Dcc 1.6.18 / Dcc.Model 1.5.10 / Render.Model 1.7.0 / Render 1.23.12, WitCloud v1.6.55). Shipped since Rev 4: gzip-packed scene submission (`Render.UnzipDccScene` + the `RenderDccScene*Packed` script family, payloads 6–10× smaller); the **binary mesh sidecar** — bulk mesh data rides a raw binary file the generated script reads back via numpy `foreach_set`, taking heavy-scene conversion from 20+ minutes to ~6 s (closes DCC-H1 root cause); full V-Ray support on the initiator side; an 8-gate production audit: **wire cutover to VersionTolerant** for `RenderSceneAttachmentRefData` + explicit `MemoryPackOrder` across Render.Model (DCC-M1 class closed; one-time break — hosts ≥ v1.6.55 pair with plugin ≥ 0.7.46), decompression cap (1 GB), Blender build wall-clock watchdog (30 min), attachment name-collision uniquification, degradation re-validation. Full 21-scene corpus (stills + video + V-Ray) green end-to-end. Still open here: DCC-H2 large-scene blob-ref path (the sidecar removed the farm-side cost; the inline submission payload remains), varying-topology sims.
**Revision 6:** 2026-07-13 — **the delegated fluid bake actually works now, in both bake strategies, with live progress.** The headless `bpy.ops.fluid.bake_all()` turned out to be a silent no-op in `-b` (flips the baked flag, writes zero cache files) — every delegated Mantaflow bake had been shipping an unbaked scene. Fixed with the REPLAY frame-stepping recipe (Render 1.23.16) in BOTH the controller bake and the addon's local bake (which additionally CRASHED Blender as a modal-in-modal operator); `Render.ValidateBlend` now treats a fluid domain as baked when its cache holds data through the configured end frame (REPLAY leaves `has_cache_baked_data` false, 1.23.17). Rigid body unblocked in both strategies — a memory-baked `rigidbody_world` cache embeds in the `.blend` like cloth (1.23.18 validator keys on the WORLD, unbaked resolves via a bake plan instead of a hard block); **never assign `use_disk_cache` on the scene-level world cache** (RNA setter segfaults Blender — the 1.23.18→1.23.19 red-box regression); a disk-cached world fails fast with guidance (1.23.20). The engine-sink shipped (Engine.Shared 1.1.6 / EngineNode 1.2.2 / WitCloud `NodeActivityProgress` / Render 1.23.21): the bake streams per-frame progress into the job's `OverallProgress`, interpolated within the current stage — live-proven (15 distinct values across a 60 s lava bake; the per-frame `DistributedProgress` channel untouched). All branches merged to `main`; worker client `client-v1.1.3-beta` adds resumable chunked blob downloads (4 MB chunks, `.partial` staging, recovery gate while a transfer progresses) so multi-GB controller updates land on slow links. Live-proven end-to-end on the lava demo (fluid + CellFracture rigid body) in both bake strategies.

---

## 0.2 Revision 4 — add-on/bridge integration plan (the "bake on submit" UX)

The controller can now bake an unbaked simulation on the network (delegated, best node) or accept a prebaked one. The add-on + bridge expose this to the user.

**Where:** `OmnibusCloud/Blender/OutWit.Render.BlenderAddon/outwit_render_bridge/` (Python add-on) + `OmnibusCloud/Blender/OutWit.Render.BlenderBridge/` (C# bridge). Add-on state is a `WindowManager` PropertyGroup (`OutWitBridgeRuntimeState`, not saved in .blend); settings with `update=_mark_render_settings_changed` persist to the **bridge store** over REST and restore on start — the new setting follows that pattern (+ a field on the bridge's render-settings DTO). The add-on has **no** local sim detection today (it string-matches the cloud validator's summary in `bridge_dependency_policy.py`).

**UX (user-decided):** a **bake-strategy choice** appears only when a simulation is detected — **Delegated (render farm, default)** or **Local (this computer)** — v1 is the strategy choice only (no resolution slider). **Hard gate:** when a sim is present the render must not start without an applied bake plan (never send an unbaked sim to a plain `Render*`). Persisted via the bridge store.

**Sequencing — 3a (farm) then 3b (local):**
- **3a Delegated.** New `bridge_simulation.py` local bpy scan (unbaked FLUID / CLOTH / SOFT_BODY / DYNAMIC_PAINT / dynamic particles / GN sim zones / rigid body). `bake_strategy` EnumProperty (default Delegated). Panel shows it + the gate; the "requires baked sim" block is bypassed when a strategy is applied. `_run_selected_launch` → new `client.run_bake_and_render_{still,frames,video,still_tiled}` → new bridge endpoints → `BakeAndRender{Mode}{Engine}` resolution (mirrors `Resolve*Script`), constructing `RenderBakeOptionsData`.
- **3b Local.** Add-on modal bake driver via `bpy.ops.*('INVOKE_DEFAULT')` so Blender's native bake progress bar + Esc-cancel show (the launch op becomes modal: bake → wait → upload → render) with add-on phase labels. Point-cache/GN bake to memory/PACKED → embedded → self-contained → `Render*`; fluid bakes to disk VDB → a **new add-on FluidCache collector** (Frame-tagged) → `Render*` prebaked path.

**Tests:** add-on (sim detect; gate blocks an unbaked sim with no strategy; submit-branch strategy×mode → correct path; local embed vs fluid collector) · bridge C# (resolve all 12 `BakeAndRender*` names; endpoint routing; settings round-trip including `bake_strategy`) · live end-to-end for both strategies. **Docs:** add-on/bridge docs, this plan, and the Render controller README (still missing the delegated-bake section).
...
**Delegated bake — BUILT controller-side 2026-06-29 (branch `feat/delegated-simulation-bake`, Render 1.23.0 / Render.Model 1.6.0; 201 non-live tests green; bake recipe validated headless on an unbaked smoke scene = 24/24 frames).** `Render.BakeSimulation` (node activity + adapter) bakes an unbaked Mantaflow fluid/gas domain to a per-frame OpenVDB cache and returns a `RenderSceneRef` carrying the baked blend + Frame-tagged `FluidCache` attachments. `BlenderBakeScript` generates the headless recipe (`cache_data_format='OPENVDB'` + `cache_type='ALL'` — critical-fields-first after a found bug: `cache_mesh_format` only accepts BOBJECT/OBJECT, setting it to OPENVDB aborted config and left REPLAY/1-frame; noise-format set is now guarded) + `BlenderRunner.BakeSimulationAsync`. `RenderBakeOptionsData`/`RenderBakeOptions` (SimulationKinds/CacheFormat/ResolutionMax). Scripts `BakeAndRender{Still,Frames,Video}Cycles` wire `Grid.Delegate() => Render.BakeSimulation(scene, start, end, bakeOptions)` then the proven `BuildBlendFromRefs → SplitBatched → ForEach(FrameBatch.Cycles) → Collect/EncodeVideo`. KEY: the bake reuses the render-throughput benchmark (`MeasureRenderAsync(Cycles)`) so `Grid.Delegate` ranks bake nodes by the SAME model as frame distribution. NEXT: merge + publish + deploy + live-prove on the real Mantaflow demo scenes, then add-on/bridge UX.
